...
object AppManager {
    ...
    fun isAppFrozen(packageName: String): Boolean = when {
        HailData.workingMode.endsWith(HailData.STOP_DISABLE) -> HPackages.isAppDisabled(packageName)
    ...
    fun setAppFrozen(packageName: String, frozen: Boolean): Boolean {
        val result = packageName != BuildConfig.APPLICATION_ID && when (HailData.workingMode) {
            ...
            HailData.MODE_SU_STOP_DISABLE -> if (frozen) {
                HShell.forceStopApp(packageName) && HShell.setAppDisabled(packageName, true)
            } else {
